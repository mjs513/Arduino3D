...
////////////////////////////////////////////////////////////////////////
//Arduino_3D::Arduino_3D(SPIClass *SPIWire, uint8_t _CS, uint8_t _DC, uint8_t _RST, uint8_t _MOSI, uint8_t _SCLK, uint8_t _MISO) : ILI9488_t3(SPIWire, _CS, _DC, _RST, _MOSI, _SCLK, _MISO)
Arduino_3D::Arduino_3D(uint8_t _CS, uint8_t _DC, uint8_t _RST, uint8_t _MOSI, uint8_t _SCLK, uint8_t _MISO) : ILI9341_t3n( _CS, _DC, _RST, _MOSI, _SCLK, _MISO) 
{
    setColorMap(11);
}
...
void Arduino_3D::get_triangle_points(Model *M, int8_t *vertices, uint8_t i, int8_t **p, int8_t **q, int8_t **r) {
...
uint8_t Arduino_3D::facing_camera(int8_t *p, int8_t *q, int8_t *r) {
...
void Arduino_3D::interpolateFlood(uint16_t x, uint16_t y, uint16_t i, uint16_t stop, uint16_t length, uint8_t color1, uint8_t color2)
{
    uint8_t flag  = DITHERED_MASK_FLAG(y);
    uint8_t alpha = 0xff;
	
    //beginSPITransaction(30000000);
    while (i<stop) {
...
/*			if(i == (stop-1)){
				//write16BitColor(c, false);
				writedata16_last(c);
			} else {
				//write16BitColor(c, true);
				writedata16_cont(c);
			}
*/
			//drawPixel(y,i+Y0,c);
...
void Arduino_3D::interpolateFastHLine(int16_t x, int16_t y, uint8_t length, uint8_t color1, uint8_t color2) {
    if (length<1) return;
        ...
        if(length<=0||y<0||y>=height()||x>=width()||x2<0) return;
        if(x<0) {length+=x; x=0;}
        if(x2>=width()) {x2=width()-1; length=x2-x+1;}
    #endif    
...
void Arduino_3D::fastFlood(uint8_t c, uint16_t l) {
    flood( 0x0101*c, l);
}

// Very fast flood routine. 
void Arduino_3D::flood(uint16_t color, uint32_t len) {
    uint8_t  i;
	beginSPITransaction(30000000);
	for(i = 0; i < len; i++) {
	  if(i == (len-1)){
...
void     Arduino_3D::overdraw_on()  {do_overdraw = 1;}
void     Arduino_3D::overdraw_off() {do_overdraw = 0;}
void     Arduino_3D::masking_on()   {do_masking  = 1;}
void     Arduino_3D::masking_off()  {do_masking  = 0;}
void     Arduino_3D::flip_mask()    {mask_flag  ^= FRAME_ID_FLAG8;}
 ...
 */
/*
void Arduino_3D::fastXORFlood(uint8_t mask, uint8_t length) {
...
*/
void Arduino_3D::fastFillScreen(uint8_t color) {
    fillScreen(color*0x0101);
}

void Arduino_3D::fastPixel(uint8_t x, uint16_t y, uint8_t color) {
    drawPixel(x,y,color*0x0101);
}

/*
void Arduino_3D::fastXORRect(uint8_t x, uint16_t y, uint8_t w, uint16_t h, uint8_t mask) {
    uint8_t  x2=x+w-1;
    SET_X_RANGE(x,x2);
    for (int i=0; i<h; i++) {
        SET_Y_LOCATION(i+y);
...
*/
void Arduino_3D::fastFillRect(uint8_t x, uint16_t y, uint8_t w, uint16_t h, uint8_t c) {
    fillRect(x,y,w,h,c*0x0101);
}

void Arduino_3D::fastDrawRect(uint8_t x, uint16_t y, uint8_t w, uint16_t h, uint8_t c) {
    drawRect(x,y,w,h,c*0x0101);
}

void Arduino_3D::fastDrawTriangle(
    int16_t x0, int16_t y0, 
    int16_t x1, int16_t y1, 
